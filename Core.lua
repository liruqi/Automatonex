...
function Automaton.modulePrototype:RegisterOptions(options)
    ...
    table.sort(self._optionOrder, function(a, b) return a.order < b.order end)
...
function Automaton:OnInitialize()
    ...
    -- 默认启停翻转迁移 v1：世界BUFF / 任务英文链接 / 升级窗口提示 / 物品链接英文转换
    -- 这批模块由"默认禁用"改为"默认启用"。老角色此前被 SetDisabledAsDefault 播种过
    -- 禁用记录，这里清掉旧播种状态，让老角色也按新默认启用（必须在本函数下方的
    -- raw 同步之前执行，同步时才不会把这批模块写回禁用）。
    -- 手动启用过的角色（disabledModules 键已被删除）不受影响；若曾在启用后手动
    -- 禁用，会随本次迁移一并重置为新默认（启用），与"默认启用"的诉求一致。
    self.db.char.defaultFlipVersion = self.db.char.defaultFlipVersion or 0
    if self.db.char.defaultFlipVersion < 1 then
        local flippedToEnabled = {
            WorldBuffs = true,
            pfQuestSendLink = true,
            NewLevelFrame = true,
            ItemLinkEnUS = true,
        }
        if self.db.char.disabledModules then
            for name in pairs(flippedToEnabled) do
                if self.db.char.disabledModules[name] then
                    self.db.char.disabledModules[name] = nil
                end
            end
        end
        if self.db.char.defaultDisabledSeeded then
            for name in pairs(flippedToEnabled) do
                self.db.char.defaultDisabledSeeded[name] = nil
            end
        end
        self.db.char.defaultFlipVersion = 1
    end
    -- 默认启停翻转迁移 v2：自动需求（AutoNeed）由"默认禁用"改为"默认启用"，
    -- 清掉老角色的旧播种禁用记录（同样须在下方 raw 同步之前执行）。
    if self.db.char.defaultFlipVersion < 2 then
        local flippedToEnabledV2 = {
            AutoNeed = true,
        }
        if self.db.char.disabledModules then
            for name in pairs(flippedToEnabledV2) do
                if self.db.char.disabledModules[name] then
                    self.db.char.disabledModules[name] = nil
                end
            end
        ...
        self.db.char.defaultFlipVersion = 2
    end
...
function Automaton:SetDisabledAsDefault(object, name, force)
    ...
    if not force then
        ...
        if not declared or declared.disabled ~= true then return end
    end
...
local function BgAlphaFor(a)
    ...
    if windowAlpha >= 1 then return a end
...
function Automaton:SetWindowAlpha(v, skipSave)
    ...
    if v < WINDOW_ALPHA_MIN then v = WINDOW_ALPHA_MIN end
    if v > 1 then v = 1 end
    windowAlpha = v
...
local function CreateFlatCheckbox(parent)
    ...
    btn:SetScript("OnClick", function()
        ...
        if btn._onChange then btn._onChange(btn.checked) end
    end)
    btn:SetScript("OnEnter", function()
        SetBg(btn, THEME.shade[1], THEME.shade[2], THEME.shade[3], 1)
    end)
...
function Automaton:RebuildMainWindow()
    ...
    local okL, left = pcall(function() return f:GetLeft() end)
    local okB, bottom = pcall(function() return f:GetBottom() end)
    if not okL then left = nil end
    if not okB then bottom = nil end
...
local function GetSettingsPanelWidth(main)
    ...
    if main and main.GetWidth then
        ...
        if ok and w and w > 0 then frameW = w end
    end
...
local function RestoreScroll(key, value)
    ...
    pcall(function()
        ...
        if sl then sl:SetValue(value) end
    end)
end
...
local function IsCursorInsideList()
    ...
    if not main or not main.listScroll then return false end
...
local function GetModuleDropIndex()
    local main = Automaton.mainFrame
    if not main or not main.listContainer then return nil end
    local scale = UIParent:GetScale()
    local x, y = GetCursorPosition()
    x = x / scale
    y = y / scale
...
local function SetDragGhostTicking(on)
    ...
    if on then
        if not f._ghostTicking then
            ...
            f:SetScript("OnUpdate", function() UpdateDragGhost() end)
        end
...
function Automaton:CreateMainWindow()
    ...
    f:SetScript("OnDragStart", function() Automaton:BeginWindowMove() end)
    f:SetScript("OnDragStop", function() Automaton:EndWindowMove() end)
    ...
    title:SetText("|cff" .. string.format("%02x%02x%02x", colorR*255, colorG*255, colorB*255) .. "Automaton 工具箱强化版|r")
    ...
    zoomOutBtn:SetScript("OnClick", function() Automaton:ZoomOut() end)
    ...
    zoomInBtn:SetScript("OnClick", function() Automaton:ZoomIn() end)
...
function Automaton:SetUIScale(scale)
    local f = self.mainFrame
    if not f then return end
    if scale < self.UI_SCALE_MIN then scale = self.UI_SCALE_MIN end
    if scale > self.UI_SCALE_MAX then scale = self.UI_SCALE_MAX end
    -- 四舍五入到两位小数，避免浮点误差累积
...
function Automaton:ReorderModuleToIndex(moduleName, targetIndex)
    ...
    if targetIndex > n then targetIndex = n end
    if cur == targetIndex then return end
...
function Automaton:PopulateModuleList()
    ...
    for name, module in pairs(self.modules) do
        if not self:IsModuleHidden(name) then
            ...
            local show = (searchText == "") or string.find(nameLower, searchText, 1, true) or string.find(displayLower, searchText, 1, true) or string.find(descLower, searchText, 1, true)
            if show then
    ...
    table.insert(modules, 1, { name = HOME_KEY, module = { modulename = L["Home"], _home = true, moduledesc = L["Home description"] } })
    ...
    for idx, entry in ipairs(modules) do
        ...
        row:SetScript("OnEnter", function()
            if Automaton._dragging then return end
...
function Automaton:ShowHomePage()
    ...
    for name, module in pairs(self.modules) do
        total = total + 1
        if self:IsModuleActive(name) then enabled = enabled + 1 end
        if self:IsModuleHidden(name) then hidden = hidden + 1 end
    end
    ...
    stats:SetText(string.format("%s：%d    %s：%d    %s：%d    %s：%d",
        L["Modules total"], total, L["Enabled count"], enabled, L["Disabled count"], total - enabled, L["Hidden count"], hidden))
    yOffset = yOffset - 22
...
function Automaton:RefreshModuleSettings(moduleName)
    ...
    AddControl = function(caption, entryKey, opt, indent)
        ...
        elseif typ == "text" then
            ...
            if opt.validate and type(opt.validate) == "table" and next(opt.validate) ~= nil then
                ...
                local function GetCurrent()
                    if not opt.get then return "" end
                    if passValue then
                ...
                local function SetValue(val)
                    if not opt.set then return end
                    if passValue then
        ...
        elseif typ == "range" or (typ == "number" and (opt.min ~= nil or opt.max ~= nil)) then
            ...
            thumb:SetScript("OnMouseUp", function() StopSliderDrag() end)
            slider:EnableMouse(true)
            slider:SetScript("OnMouseUp", function() StopSliderDrag() end)
        ...
        elseif typ == "execute" then
            ...
            if btnWidth < 50 then btnWidth = 50 end
            if btnWidth > 200 then btnWidth = 200 end
...
if DEFAULT_CHAT_FRAME then
    DEFAULT_CHAT_FRAME:AddMessage("|cffffcc00[|r|cff00ffffAutomaton|r|cffffcc00]|r 设置界面已加载，输入 /auto 打开配置窗口")
end