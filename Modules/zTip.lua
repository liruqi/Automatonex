...
Automaton_zTip.moduledesc = "鼠标提示增强，显示更多信息（职业图标、军衔、采集等级、抗性等）"

-- 采集数据（保持不变）
local gatheringData = {
    -- 草药
    ['宁神花'] = 1, ['银叶草'] = 1, ['地根草'] = 15, ['魔皇草'] = 50, ['石南草'] = 70,
    ['荆棘藻'] = 85, ['跌打草'] = 100, ['野钢花'] = 115, ['墓地苔'] = 120, ['皇血草'] = 125,
    ['活根草'] = 150, ['枯叶草'] = 160, ['金棘草'] = 170, ['卡德加的胡须'] = 185,
    ['冬针草'] = 195, ['火焰花'] = 205, ['紫莲花'] = 210, ['太阳草'] = 230, ['盲目草'] = 235,
    ['幽灵菇'] = 245, ['格罗姆之血'] = 250, ['阿尔萨斯之泪'] = 255, ['黄金参'] = 260,
    ['梦叶草'] = 270, ['山鼠草'] = 280, ['瘟疫花'] = 285, ['冰盖草'] = 290, ['黑莲花'] = 300,
    -- 矿物
    ['铜矿'] = 1, ['锡矿'] = 65, ['银矿'] = 75, ['铁矿'] = 125, ['铁矿石'] = 125,
    ['金矿'] = 155, ['金矿石'] = 155, ['秘银矿脉'] = 175, ['真银矿脉'] = 230,
...
function Automaton_zTip:FormatUnit(unit)
    ...
    for i = 2, GameTooltip:NumLines() do
        ...
        if tip then
            ...
            elseif string.find(tip, LEVEL) then
                if not levelline then levelline = i end
    ...
    if levelline then
        ...
        tmp2 = ""
        if isplayer then
            if zShowIsPlayer then tmp2 = format("(%s)", PLAYER) end
...
function Automaton_zTip:ShowGatheringLevel()
    if not zShowGatheringLevel then return end
    ...
    if gatherLevel then
        ...
        if r > 0.8 and g < 0.3 then return end
        local tiptext = GameTooltipTextLeft2:GetText()
        if tiptext then GameTooltipTextLeft2:SetText(tiptext.." "..gatherLevel) end
        GameTooltip:Show()
    end
end
...
function Automaton_zTip:ShowDamageAndSpeed(unit)
    if not zShowDamageAndSpeed then return end
    local reaction = UnitReaction(unit, "player")
...
function Automaton_zTip:ShowImpression(unit)
    if not zShowImpression then return end
...
function Automaton_zTip:CheckChallenges(unit)
    if PLAYER_CHALLENGES == "0" or not unit or not UnitIsPlayer(unit) or UnitIsUnit("target", "player") then return end
...
function Automaton_zTip:Create3DModelFrame()
    if self.modelFrame then return end
    ...
    if p.zModelRotation then
        ...
        self.modelFrame:SetScript("OnUpdate", function(frame, elapsed)
            if not self.modelRotationEnabled then return end
            if not elapsed then return end  -- 防御：如果elapsed为nil则退出
        ...
        self.modelFrame:SetScript("OnEnter", function() self.modelRotationEnabled = true end)
        self.modelFrame:SetScript("OnLeave", function() self.modelRotationEnabled = false end)
    else
...
function Automaton_zTip:UpdateModelPosition()
    if not self.modelFrame or not self.modelFrame:IsShown() then return end
    local p = self.db.profile
    ...
    if p.zModelEdge then
        ...
        local left, right, bottom, top = self.modelFrame:GetLeft(), self.modelFrame:GetRight(), self.modelFrame:GetBottom(), self.modelFrame:GetTop()
        if left and right and bottom and top then
            local offsetX, offsetY = 0, 0
            if left < 0 then offsetX = -left + 5 end
            if right > screenWidth then offsetX = screenWidth - right - 5 end
            if bottom < 0 then offsetY = -bottom + 5 end
            if top > screenHeight then offsetY = screenHeight - top - 5 end
...
function Automaton_zTip:ShowModelForUnit(unit)
    ...
    if not p.zShow3DModel then
        if self.modelFrame then self.modelFrame:Hide() end
        return
    end
...
function Automaton_zTip:UpdateModelConfig()
    if not self.modelFrame then return end
    local p = self.db.profile
    self.modelFrame:SetWidth(p.zModelSize)
    self.modelFrame:SetHeight(p.zModelSize)
    if p.zModelRotation then
...
function Automaton_zTip:OnEnable()
    ...
    moduleFrame:SetScript("OnUpdate", function(self, elapsed) Automaton_zTip:OnUpdate(elapsed) end)
    ...
    if not GameTooltip:GetScript("OnEvent") then
        GameTooltip:SetScript("OnEvent", function(self, event, ...) Automaton_zTip:OnTooltipEvent(event) end)
    end

    -- 保存原始函数并覆写
    self.orig_GameTooltip_UnitColor = GameTooltip_UnitColor
    GameTooltip_UnitColor = function(unit) return Automaton_zTip:FormatUnit(unit) end
    ...
    UnitFrame_OnLeave = function()
        ...
        zIsOnUnitFrame = nil
        if z_ClassIcon then z_ClassIcon:Hide() end
        if RankIcon then RankIcon:Hide() end
    ...
    if zAnchor ~= 0 then
        GameTooltip_SetDefaultAnchor = function(tooltip, parent) Automaton_zTip:SetDefaultAnchor(tooltip, parent) end
    else
...
function Automaton_zTip:OnDisable()
    ...
    -- 恢复物品ID钩子
    local hooks = {
        "SetHyperlink", "SetBagItem", "SetQuestLogItem", "SetQuestItem",
        "SetLootItem", "SetInventoryItem", "SetPlayerBuff", "SetPlayerDebuff",
...
function Automaton_zTip:RefreshTooltipDisplay()
    ...
    if zShowBar then
        ...
        if ManaBar then ManaBar:Show() end
    else
        GameTooltipStatusBar:Hide()
        if ManaBar then ManaBar:Hide() end
    end
    if zShowBarText then
        if tooltipStatusBar and tooltipStatusBar.HP then tooltipStatusBar.HP:Show() end
        if ManaBar and ManaBar.MP then ManaBar.MP:Show() end
    else
        if tooltipStatusBar and tooltipStatusBar.HP then tooltipStatusBar.HP:Hide() end
        if ManaBar and ManaBar.MP then ManaBar.MP:Hide() end
    end
...
function Automaton_zTip:InitBars()
    ...
    if not ManaBar then
        ...
        local function ManaBar_Update()
            if not GameTooltip.unit then return end
