...
setmetatable(M, { __index = function(M, key) return L[key] end })
setmetatable(G, { __index = function(G, key) return F[key] end })
...
local mapProf = {
	...
	["Ironforge"] = {
		...
		["12"] = {
			...
			["title"] = L["Riding"],
			["note"] =
				L["Amberstill Ranch"] .. " @ " .. "63.94,50.10",
		...
		["13"] = {
			...
			["title"] = L["Riding"],
			["note"] =
				L["Steelgrill's Depot"] .. " @ " .. "49.15,48.13",
	...
	["Orgrimmar"] = {
		...
		["5"] = {
			...
			["title"] = L["Alchemy"],
			["shop"] =
				L["Yelmak's Alchemy and Potions"],
			["trainers"] = {
	...
	["Stormwind"] = {
		...
		["18"] = {
			...
			["title"] = L["Riding"],
			["note"] =
				L["Eastvale Logging Camp"] .. " @ " .. "84.32,64.87",
	...
	["TheExodar"] = {
		...
		["14"] = {
			...
			["title"] = L["Riding"],
			["note"] =
				L["East of the main entrance"] .. " @ " .. "81.33,52.63",
	...
	["ThunderBluff"] = {
		...
		["13"] = {
			...
			["title"] = L["Riding"],
			["note"] =
				L["Bloodhoof Village"] .. " @ " .. "47.64,58.47",
...
local mapScales = { -- max out                           max in            max out                           max in
	["Darnassus"] = { ["x"] = { 0.188, 0.190, 0.190, 0.189, 0.188, 0.191 }, ["y"] = { 0.286, 0.279, 0.285, 0.280, 0.283, 0.284 }, },
	...
	["Orgrimmar"] = { ["x"] = { 0.093, 0.086, 0.078, 0.064, 0.056, 0.054 }, ["y"] = { 0.138, 0.128, 0.117, 0.096, 0.085, 0.080 }, },
...
local function ShowOnWorldMap(on)
	if not WorldMapFrame:IsVisible() or on then return end
...
local function Automaton_Trainer_OnUpdate()
			...
			if (mini.zoneText == L[k]) then
				...
				elseif (mapHackResult > 0) then
					if (mapHackResult == 4) then
						radiusByScaleXC = mini.radii[mini.zoom + 1] *
						mapScales["TerraceOfLight"]["x"][mini.zoom + 1]
						radiusByScaleYC = mini.radii[mini.zoom + 1] *
						mapScales["TerraceOfLight"]["y"][mini.zoom + 1]
					else
	...
	if (mapHackResult >= 0) and (TrainerDB.hideProfIcons ~= "y") then
		...
		for k, v in pairs(numMapProf) do
			...
			if (mini.zoneText == L[k]) then
				if (mapHackResult > 0) then
					if (mapHackResult == 4) then
						radiusByScaleXP = mini.radii[mini.zoom + 1] *
						mapScales["TerraceOfLight"]["x"][mini.zoom + 1]
						radiusByScaleYP = mini.radii[mini.zoom + 1] *
						mapScales["TerraceOfLight"]["y"][mini.zoom + 1]
					else
	...
	if (mini.entered == true) and (validArea == true) then
		...
		cursorX, cursorY = cursorX / scale - Minimap:GetLeft() - mini.width / 2,
			cursorY / scale - mini.height / 2 - Minimap:GetBottom()
		local pixels = 7.5 * TrainerDB.iconSize /
		21.5                                     -- Seems about right, even if ever so slightly off the visible centre
...
SlashCmdList["Trainer"] = function(options)
	...
	if not options1 or (options1 == "?") then
		printPC(pc_colour_Highlight .. L["Commands"] .. ":\n" ..
			pc_colour_Highlight .. "s n" .. pc_colour_PlainText .. " = " ..
			L["Icon size"] .. ". " .. L["10 to 50"] .. "\n" ..
			pc_colour_Highlight .. "c" .. pc_colour_PlainText .. " = " ..
			L["Class icons"] .. ". (" .. L["Toggle"] .. ")\n" ..
			pc_colour_Highlight ..
			"p" .. pc_colour_PlainText .. " = " .. L["Profession icons"] .. ". (" .. L["Toggle"] .. ")\n" ..
			pc_colour_Highlight ..
			"w" .. pc_colour_PlainText .. " = " .. L["World Map icons"] .. ". (" .. L["Toggle"] .. ")\n" ..
			pc_colour_Highlight .. "m" ..
			pc_colour_PlainText .. " = " .. L["Minimap icons"] .. ". (" .. L["Toggle"] .. ")\n" ..
	...
	elseif (options1 == "c") then
		...
		printPC("/trainer c = " ..
		pc_colour_Highlight .. ((TrainerDB.hideClassIcons == "y") and L["Hide"] or L["Show"]))
		if secondParm then
	...
	elseif (options1 == "p") then
		...
		printPC("/trainer p = " ..
		pc_colour_Highlight .. ((TrainerDB.hideProfIcons == "y") and L["Hide"] or L["Show"]))
		if secondParm then
			printPC("(" .. L["Default"] .. " = " .. L["Show"] .. ")")
		end
		ShowOnWorldMap()
	elseif (options1 == "w") then
		...
		printPC("/trainer w = " ..
		pc_colour_Highlight .. ((TrainerDB.hideWorldMapIcons == "y") and L["Hide"] or L["Show"]))
		if secondParm then
			printPC("(" .. L["Default"] .. " = " .. L["Show"] .. ")")
		end
		ShowOnWorldMap()
	elseif (options1 == "m") then
		...
		printPC("/trainer m = " ..
		pc_colour_Highlight .. ((TrainerDB.hideMinimapIcons == "y") and L["Hide"] or L["Show"]))
		if secondParm then
			printPC("(" .. L["Default"] .. " = " .. L["Show"] .. ")")
		end
		ShowOnWorldMap()
